...
2. `ASANTEPDF_MASTER_UPGRADE_SPEC.md` in full
...
8. the current Windows validation workflow and release workflow when changing engineering or packaging
...
Every numbered item in the 49-item contract is an acceptance item. Keep `IMPLEMENTATION_MATRIX.md` and `docs/PROJECT_STATE.md` current.
...
When asked to continue AsantePDF, do not invent a new roadmap and do not stop at item 45. Read the files above, inspect the current implementation matrix and project state, continue from the highest-priority incomplete dependency, validate each coherent batch on Windows, promote clean source, then continue.

Do not declare AsantePDF 1.0 final while any material requirement among all 49 remains unaccepted.