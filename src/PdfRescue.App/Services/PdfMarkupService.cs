...
public sealed class PdfMarkupService
{
    ...
    public Task StampImageAsync(string inputPath, string outputPath, int pageNumber, NormalizedPdfRect area, string imagePath, CancellationToken token = default) =>
        ModifyPageAsync(inputPath, outputPath, pageNumber, (page, gfx) =>
        {
            if (!File.Exists(imagePath)) throw new FileNotFoundException("Signature image was not found.", imagePath);
            var r = ToPageRect(page, area.Clamp());
            if (r.Width < 2 || r.Height < 2) throw new ArgumentException("The signature area is too small.", nameof(area));
            using var image = XImage.FromFile(imagePath);
            gfx.DrawImage(image, r);
        }, token);

    public Task AddRectangleAsync(string inputPath, string outputPath, int pageNumber, NormalizedPdfRect area, CancellationToken token = default) =>
        ModifyPageAsync(inputPath, outputPath, pageNumber, (page, gfx) =>
        {
            var r = ToPageRect(page, area.Clamp());
            if (r.Width < 2 || r.Height < 2) throw new ArgumentException("The rectangle is too small.", nameof(area));
            gfx.DrawRectangle(new XPen(XColor.FromArgb(255, 31, 111, 235), 2), r);
        }, token);

    public Task AddEllipseAsync(string inputPath, string outputPath, int pageNumber, NormalizedPdfRect area, CancellationToken token = default) =>
        ModifyPageAsync(inputPath, outputPath, pageNumber, (page, gfx) =>
        {
            var r = ToPageRect(page, area.Clamp());
            if (r.Width < 2 || r.Height < 2) throw new ArgumentException("The ellipse is too small.", nameof(area));
            gfx.DrawEllipse(new XPen(XColor.FromArgb(255, 31, 111, 235), 2), r);
        }, token);
    ...
    public async Task PermanentRedactAsync(string inputPath, string outputPath, int pageNumber, NormalizedPdfRect area, CancellationToken token = default)
    {
        ...
        var tempDir = Path.Combine(Path.GetTempPath(), "AsantePDF", "redaction", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempDir);
        ...
        CancellationToken token)
    {
        return Task.Run(() =>
        {
            ...
            var output = Path.GetFullPath(outputPath);
            ValidatePaths(input, output);
            var tempDir = Path.Combine(Path.GetTempPath(), "AsantePDF", "markup", Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(tempDir);
    ...
    private static bool NormalizeWithQpdf(string inputPath, string outputPath, CancellationToken token)
    {
        ...
        {
            ExternalProcessCleanup.TerminateAndDrain(
