...
| 17 | Create an application-wide task/progress framework | IN PROGRESS | Foreground operations remain centrally tracked through `RunBusyAsync`, while `BackgroundTaskQueueService` now gives eligible work genuine queued/running/completed/failed/cancelled execution without globally locking the workspace. Compression, Repair, Web Optimize and Unlock queue independently from document tabs. Cancellation is graceful; dirty page layouts are captured into isolated temporary working PDFs and cleaned afterward. Queue serialization and queued cancellation are covered by smoke tests. Rich page/item counts and background routing for OCR/conversion and other long operations remain. |
| 18 | Add a Task Center | IN PROGRESS | Dedicated Task Center shows Running, Queued, Completed, Failed and Cancelled jobs with filtering, elapsed time, progress, errors, Cancel, Open Result and Retry for retry-safe failures/cancellations. The core single-worker queue now supports multiple real queued jobs while users continue in other tabs. Sensitive password jobs are deliberately non-retryable so credentials are not retained in retry delegates. OCR/conversion and the rest of the long-running tool set still need migration onto the same queue. |
| 19 | Create proper completion workflows | IN PROGRESS | `OperationResultDialog` and shared completion routing distinguish original/result files and provide Open in New Tab, safe Use Result Here, Open Folder, Save a Copy, Run Another and Close. Compression, Repair, Web Optimization and Unlock are wired and Windows validated. Background completions remain non-interrupting and expose their outputs through Task Center. The pattern still needs expansion to merge/split/OCR/conversion/security/annotation and other output-producing operations. |
| 20 | Default to non-destructive processing | REVIEW REQUIRED | RC10 has strong non-destructive behavior, and the background queue now snapshots dirty layouts into temporary copies rather than mutating source PDFs. The complete operation-by-operation audit remains. |
...
| 32 | Add batch processing | REVIEW REQUIRED | The new Task Center queue is deliberately the execution foundation for future batch workflows; batch UI/rules are not yet accepted. |
...
| 37 | Improve performance perception | REVIEW REQUIRED | Recent first-page thumbnails are generated asynchronously and cached by file identity; eligible transformations can now run through Task Center without freezing or globally locking the workspace. Wider performance-perception work remains. |
...
| 42 | Establish one coherent application architecture | IN PROGRESS | AsantePDF now has three connected experiences in code: Home / Launcher, a true multi-document Document Workspace, and a Task Center backed by a real application queue. Shared result routing and independent queued execution are now present. Remaining architecture work is broad migration of long-running tools plus search/bookmarks/settings and other master-spec systems. |
| 43 | Maintain the release-quality engineering standard | IMPLEMENTED, NOT ACCEPTED | RC10 full Windows release gate passed end to end. Architecture Batch 1 passed run 32647383215. Recent/session integration passed run 32648130725, Starred/Resume passed run 32648422410, and later architecture iterations were repeatedly validated through run 32649073796. Background-queue job `97232413507` passed patch application, exact .NET 10.0.202 x64 Release compilation, expanded core smoke tests including queue serialization/cancellation, and validated source commit. Full installer/installed-copy gate must be rerun for a release candidate. |
