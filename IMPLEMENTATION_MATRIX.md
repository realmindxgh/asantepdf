...
| 21 | Redesign PDF Doctor | IMPLEMENTED, NOT ACCEPTED | Initial state is honest and the real Run PDF Doctor action now has a professional icon. Diagnosis presents Healthy / Attention Needed / Damaged states, grouped findings with Structure/Security/Optimization/Performance categories, surfaced qpdf engine messages, and contextual Repair PDF / Compress PDF actions only for supported findings. Structural errors/warnings, encryption, file size, page count and PDF version remain the measured evidence. Staged jobs `32726442308` / `32727012569`; clean jobs `32726666914` / `32727205478`; clean promoted source `8500dcaae8bddf868a33850da57a9d47918fa70a`. Hands-on Doctor/Inspector visual and runtime acceptance remains. |
| 22 | Redesign the Inspector | IN PROGRESS | Document-details/Doctor shell exists. Context-specific page/text/annotation/object modes are being audited and extended. The first carrier `260-inspector-context.ps1` failed during patch application because its assumed annotation-selection target was not present, and was removed without promoting item-22 source changes. Replacement carrier `261-inspector-context-safe.ps1` is now staged, using stable page-selection and annotation-navigation targets only. |
...
| 43 | Maintain the release-quality engineering standard | IMPLEMENTED, NOT ACCEPTED | RC10 full release gate is proven. Development changes are repeatedly staged on Windows, compiled/smoke-tested, promoted to ordinary source, then rerun without patch carriers. Latest item-21 staged gates `32726442308` / `32727012569` and clean no-patch gates `32726666914` / `32727205478` used exact .NET `10.0.202`, built with 0 warnings/0 errors and passed all smoke tests. Clean-proven ordinary source is `8500dcaae8bddf868a33850da57a9d47918fa70a`; the branch may advance with documentation/provenance commits without changing that source tree. Full installer/installed-copy gate must be rerun at release-candidate stage. |
